...
统一变速命令，支持倍速和帧率两种模式。当目标帧率超过 50fps 时自动使用抽帧实现。

- **指令:** `/gif变速 [参数]`
- **使用方法:** 回复一个GIF并发送指令。
- **参数:**
    - **倍速模式:** `2x` / `0.5x` — 2倍速或半速播放
    - **帧率模式:** `30fps` — 设置目标帧率为30帧/秒
- **抽帧提示:** 当变速后帧率超过 50fps 时，自动抽帧实现并提示用户。

**示例:** `/gif变速 2x`、`/gif变速 0.5x`、`/gif变速 30fps`
